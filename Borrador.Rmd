...
```{r}
df_ori <- read_delim("./data/Indicadores.csv", delim = ";", escape_double = FALSE, locale = locale(decimal_mark = ",", grouping_mark = "."), trim_ws = TRUE)

na_diff <- which(!is.na(df_ori$Total) & is.na(df$Total))
diff_observations <- df_ori[na_diff, , drop = FALSE]
diff_observations
```

Parece ser que, para algunos municipios, al no disponerse el valor del indicador se codifica como "..". (Consultar razón específica de esta codificación)
