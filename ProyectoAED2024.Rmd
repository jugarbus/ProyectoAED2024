...
g1 <- ggplot(data = map_municipios_2010) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray") +  # Escala de color
  theme_minimal() +
  labs(title = "Tasa de desempleo (Porcentaje)",
       subtitle = "Año 2010",
       fill = "Porcentaje",
...
min_value <- mun_data %>% filter(Indicadores == "Tasa de desempleo (Porcentaje)") %>% select(Total) %>% min(na.rm = T)
max_value <- mun_data %>% filter(Indicadores == "Tasa de desempleo (Porcentaje)") %>% select(Total) %>% max(na.rm = T)
...
g1 <- ggplot(data = map_municipios_2010) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray", limits = c(min_value, max_value), direction = -1) +  # Mismo rango de colores
  theme_minimal() +
  labs(title = "Tasa de desempleo (Porcentaje)",
       subtitle = "Año 2010",
       fill = "Porcentaje",
...
g3 <- ggplot(data = map_municipios_2016) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray", limits = c(min_value, max_value), direction = -1) +  # Escala de color
  theme_minimal() +
  labs(title = "Tasa de desempleo (Porcentaje)",
       subtitle = "Año 2016",
       fill = "Porcentaje",
...
mun_data_2016 <- mun_data %>%
  filter(Periodo == as.Date("2021-01-01"),
         Indicadores == "Tasa de desempleo (Porcentaje)")
mun_data_2019 <- mun_data %>%
  filter(Periodo == as.Date("2022-01-01"),
         Indicadores == "Tasa de desempleo (Porcentaje)")
# Find the min and max values of the indicator for both 2010 and 2013
min_value <- mun_data %>% filter(Indicadores == "Tasa de desempleo (Porcentaje)") %>% select(Total) %>% min(na.rm = T)
max_value <- mun_data %>% filter(Indicadores == "Tasa de desempleo (Porcentaje)") %>% select(Total) %>% max(na.rm = T)
...
mun_data_2010 <- mun_data %>%
  filter(Periodo == as.Date("2019-01-01"),
         Indicadores == "Proporción de empleo en industria (NACE Rev.2 B-E) (Porcentaje)")
mun_data_2013 <- mun_data %>%
  filter(Periodo == as.Date("2020-01-01"),
         Indicadores == "Proporción de empleo en industria (NACE Rev.2 B-E) (Porcentaje)")
mun_data_2016 <- mun_data %>%
...
mun_data_2019 <- mun_data %>%
  filter(Periodo == as.Date("2022-01-01"),
         Indicadores == "Proporción de empleo en industria (NACE Rev.2 B-E) (Porcentaje)")
# Find the min and max values of the indicator for both 2010 and 2013
min_value <- mun_data %>% filter(Indicadores == "Proporción de empleo en industria (NACE Rev.2 B-E) (Porcentaje)") %>% select(Total) %>% min(na.rm = T)
max_value <- mun_data %>% filter(Indicadores == "Proporción de empleo en industria (NACE Rev.2 B-E) (Porcentaje)") %>% select(Total) %>% max(na.rm = T)
...
```{r}
mun_data %>% filter(Indicadores == "Población residente (Personas)") %>% select(Total) %>% min(na.rm = T) %>% print()
mun_data %>% filter(Indicadores == "Población residente (Personas)") %>% select(Total) %>% max(na.rm = T) %>% print()
```

```{r}
# Crear directorio para guardar los mapas
output_dir <- "graficos_mapas_ajustado"
...
for (indicador in indicadores) {
  ...
  min_value <- mun_data %>% filter(Indicadores == indicador) %>% select(Total) %>% min(na.rm = T)
  max_value <- mun_data %>% filter(Indicadores == indicador) %>% select(Total) %>% max(na.rm = T)
  ...
  for (fecha in fechas_bucle) {
    ...
    map <- ggplot(data = map_municipios_generico_fecha) +
      ...
      scale_fill_viridis_c(option = "rocket", na.value = "lightgray",limits = c(min_value, max_value)) +
      theme_minimal() +
