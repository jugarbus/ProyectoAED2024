...
```{r diagramas_dispersion, fig.width=15, fig.height=10, fig.cap= "\\label{fig:diagramas_dispersion}Diagramas de dispersión para los datos por municipios.", message=FALSE}
...
paste0("Municipios que no coinciden: ", paste(names_not_matched, collapse = ", "))

```

Estos municipios son aquellos de los que tenemos datos, pero cuyo nombre no coincide con los de la librería mapSpain. Por lo tanto, mapeamos en nuestro dataset los nombres actuales a los que coinciden en dicha librería.
...
g3 <- ggplot(data = map_municipios_2010_ext) +
  ...
  theme_minimal() +
  labs(title = "Porcentaje de Extranjeros",
       subtitle = "Año 2010",
       fill = "Porcentaje")
...
Se han representado los mapas correspondientes a la variable "Proporción de empleo en servicios (NACE Rev.2 G-U) (Porcentaje)" para los años 2019, 2020 y 2021. A través de estas representaciones, es posible observar el impacto del COVID-19 en esta variable.
...
mun_data_2010_des <- mun_data_corrected %>%
  filter(Periodo == as.Date("2019-01-01"),
         Ind == "Emp_Servicios")
mun_data_2019_des <- mun_data_corrected %>%
  filter(Periodo == as.Date("2020-01-01"),
         Ind == "Emp_Servicios")
mun_data_2010_ext <- mun_data_corrected %>%
  filter(Periodo == as.Date("2021-01-01"),
         Ind == "Emp_Servicios")

# Find the min and max values of the indicator for both 2010 and 2013
min_value_des <- mun_data_corrected %>% filter(Ind == "Emp_Servicios") %>% pull(Total) %>% min(na.rm = TRUE)
max_value_des <- mun_data_corrected %>% filter(Ind == "Emp_Servicios") %>% pull(Total) %>% max(na.rm = TRUE)

min_value_act <- mun_data_corrected %>% filter(Ind == "Emp_Servicios") %>% pull(Total) %>% min(na.rm = TRUE)
max_value_act <- mun_data_corrected %>% filter(Ind == "Emp_Servicios") %>% pull(Total) %>% max(na.rm = TRUE)
