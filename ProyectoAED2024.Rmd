...
```{r}
# Supongamos que tienes mun_data cargado en tu entorno
# Filtrar por el año 2017 y un indicador específico
mun_data_2017 <- mun_data %>%
  filter(Periodo == as.Date("2010-01-01"),
         Indicadores == "Tasa de desempleo (Porcentaje)")

# Descargar los datos espaciales de municipios
map_municipios <- esp_get_munic()

# Unir los datos espaciales con los datos filtrados
map_municipios <- map_municipios %>%
  left_join(mun_data_2017, by = c("name" = "Municipios"))
map_municipios <- map_municipios %>%
