...
status: submit
#abstract: |
#  El presente artículo consiste en un proyecto de la asignatura *Análisis Exploratorio de Datos*, en el que se realiza un análisis exploratorio del conjunto de datos de Indicadores Urbanos, publicado por el Instituto Nacional de Estadística (INE). Dicho conjunto de datos está compuesto por 3 subconjuntos de indicadores de distinto ámbito, siendo estos indicadores demográficos, indicadores sociales e indicadores económicos. A lo largo del mismo se plantean y responden preguntas de interés sobre aspectos demográficos, económicos y sociales de España en los últimos 15 años.
# back matter
keywords: |
...
El análisis comienza con la importación y concatenación de los subconjuntos de datos, seguido de un estudio previo de la estructura del *dataset* así como de sus peculiaridades. A continuación, se lleva a cabo la imputación de los valores faltantes y se procede con el análisis univariante y el análisis bivariante. "Debido a que los datos tienen una componente geográfica, el municipio al que pertenece indicador calculado, se procede a la generación de mapas mediante los cuales poder observar la evolución de los distintos indicadores a lo largo del tiempo. Finalmente, se responden a las preguntas planteadas empleando los análisis realizados previamente y se extraen las conclusiones del proyecto.
...
* ¿Existen diferencias de indicadores geográficamente?

* ¿Qué indicadores demográficos están correlacionados con la tasa de desempleo?
...
g1 <- full_df %>% filter(Municipios=='Total Nacional') %>% 
  ...
  ggplot(aes(x = reorder(Sexo, n), y = n)) +
    ...
    labs(
      title = "Conteo NAs por Sexo del dataset total",
      x = "Sexo",
      y = "Cantidad de NA"
    ) +
    theme_minimal()
...
```{r}
kable(n_na_sex, caption = "Porcentajes de NAs por sexo en el dataset total\\label{total_sex_na}", col.names = c("Sexo", "Cantidad de NAs", "Porcentaje", "Porcentaje total"))
...
kable(n_na_sex_2, caption = "Porcentajes de NAs por sexo en el dataset por municipios\\label{mun_sex_na}", col.names = c("Sexo", "Cantidad de NAs", "Porcentaje", "Porcentaje total"))
...
```{r}
kable(p_total, caption = "Porcentajes de NAs conjunto total\\label{total_na_post}", col.names = c("Porcentaje"))
```


```{r}
kable(p_mun, caption = "Porcentajes de NAs por municipios\\label{mun_na_post}", col.names = c("Porcentaje"))
```
...
Para el dataset total se calcularon tanto la media como la mediana de *Total* para cada grupo de *Indicadores*. Luego, se compararon estas dos medidas y se observó que en ciertos indicadores la diferencia entre la media y la mediana era significativa. Debido a estas discrepancias, se decidió utilizar la mediana para la imputación, ya que es menos sensible a los valores atípicos.
...
ggplot(mun_univ_transformed, aes(x = Indicadores, y = Total)) +
  ...
  theme_minimal() +
  labs(
    title = "Boxplot de Indicadores por municipios",
    x = "Indicador",
    y = "Total"
  ) +
  theme(
...
ggplot(total_univ_transformed, aes(x = Indicadores, y = Total)) +
  ...
  theme_minimal() +
  labs(
    title = "Boxplot de Indicadores de Total Nacional",
    x = "Indicador",
    y = "Total"
  ) +
  theme(
...
anexo1_2 <- kable(estadisticos, 
             digits = 1, 
             caption = "Estadísticos dataset total\\label{tabla:anexo1_2}")
...
```{r diagramas_dispersion, fig.width=14, fig.height=8, fig.cap= "\\label{fig:diagramas_dispersion}Diagramas de dispersión para los datos por municipios.", message=FALSE}
...
## Visualizacion con mapas

Para visualizar los datos de toda España, utilizaremos la librería mapSpain. Esta herramienta nos permite representar diversos indicadores sobre el mapa de España. Visualizar los datos en un mapa puede ayudarnos a comprender las correlaciones entre variables y sus evoluciones, además de facilitarnos la tarea de identificar las provincias de las que no disponemos de datos. Dado que contamos con datos de un número reducido de municipios en comparación con el total de municipios en España (8.132), representaremos los datos a nivel provincial, calculando la media para cada una de ellas. El primer paso consistirá en verificar que los nombres de los municipios en la base de datos con la que estamos trabajando coinciden con los proporcionados por la librería y resolver cualquier discrepancia que pueda existir. Para ello, he modificado los nombres de los municipios en el dataset, cambiándolos por los nombres correctos de los municipios proporcionados, de manera que coincidan.
...
t <- paste(names_not_matched, collapse = ", ")
texto <- paste0("Municipios que no coinciden: ", paste(names_not_matched, collapse = ", "))
texto_envuelto <- paste(strwrap(texto, width = 80), collapse = "\n")
...
En el apartado anterior, hemos analizado gráficamente variables que mostraban correlaciones significativas. Sin embargo, también es relevante considerar aquellos casos en los que dos variables no presentan correlación, ya que esto puede proporcionar información igualmente valiosa. Tal como se observa en la matriz de correlación y se mencionó previamente respecto a la "Tasa_Desempleo", las variables "Tasa_Desempleo" (tasa de desempleo) y "Ext_Total" (porcentaje de extranjeros) no muestran una relación significativa.

A través de mapas, podemos examinar el comportamiento de estos dos indicadores en algunos años específicos. Para la variable "Tasa_Desempleo", se observa que su distribución evoluciona en función de la latitud: la tasa de desempleo es mayor a medida que avanzamos hacia el sur de España. Por otro lado, la variable "Ext_Total" evoluciona con respecto a la longitud: el porcentaje de extranjeros aumenta a medida que nos desplazamos hacia el este del país. Observamos que no tenemos datos en: Huesca, Segovia, Soria y Teruel


```{r, echo = FALSE}
mun_data_2010_des <- mun_data_corrected %>%
  filter(Periodo == as.Date("2010-01-01"),
         Ind == "Tasa_Desempleo")
mun_data_2019_des <- mun_data_corrected %>%
  filter(Periodo == as.Date("2019-01-01"),
         Ind == "Tasa_Desempleo")
mun_data_2010_ext <- mun_data_corrected %>%
  filter(Periodo == as.Date("2010-01-01"),
         Ind == "Ext_Total")
mun_data_2019_ext <- mun_data_corrected %>%
  filter(Periodo == as.Date("2019-01-01"),
         Ind == "Ext_Total")
# Find the min and max values of the indicator for both 2010 and 2013
min_value_des <- mun_data_corrected %>% filter(Ind == "Tasa_Desempleo") %>% pull(Total) %>% min(na.rm = TRUE)
max_value_des <- mun_data_corrected %>% filter(Ind == "Tasa_Desempleo") %>% pull(Total) %>% max(na.rm = TRUE)

min_value_act <- mun_data_corrected %>% filter(Ind == "Ext_Total") %>% pull(Total) %>% min(na.rm = TRUE)
max_value_act <- mun_data_corrected %>% filter(Ind == "Ext_Total") %>% pull(Total) %>% max(na.rm = TRUE)

# Plot for 2010
map_municipios_2010_des <- map_municipios %>%
  left_join(mun_data_2010_des, by = c("name" = "Municipios"))
map_municipios_2010_des <- map_municipios_2010_des %>%
...
# Plot for 2013
map_municipios_2016_des <- map_municipios %>%
  left_join(mun_data_2019_des, by = c("name" = "Municipios"))
map_municipios_2016_des <- map_municipios_2016_des %>%
  group_by(ine.prov.name) %>%
  mutate(Total = mean(Total, na.rm = TRUE)) %>%
  group_by(ine.prov.name, Total) %>%
  summarise(geometry = st_union(geometry), .groups = "drop")
...
m1 <- map_municipios_2010_des %>% pull(Total) %>% min(na.rm = TRUE)
m2 <- map_municipios_2016_des %>% pull(Total) %>% min(na.rm = TRUE)
min_value_act <- min(m1, m2)
m3 <- map_municipios_2010_des %>% pull(Total) %>% max(na.rm = TRUE)
m4 <- map_municipios_2016_des %>% pull(Total) %>% max(na.rm = TRUE)
max_value_act <- max(m3, m4)


g1 <- ggplot(data = map_municipios_2010_des) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray", limits = c(min_value_des, max_value_des), direction = -1) +  # Mismo rango de colores
  theme_minimal() +
  labs(title = "Tasa de desempleo, Año 2010",
       fill = "Porcentaje") +
  theme(plot.title = element_text(size = 10),
    legend.title = element_text(size = 8))

g2 <- ggplot(data = map_municipios_2016_des) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray", limits = c(min_value_des, max_value_des), direction = -1) +  # Mismo rango de colores
  theme_minimal() +
  labs(title = "Tasa de desempleo, Año 2019",
       fill = "Porcentaje") +
  theme(plot.title = element_text(size = 10),
    legend.title = element_text(size = 8))

map_municipios_2010_ext <- map_municipios %>%
  left_join(mun_data_2010_ext, by = c("name" = "Municipios"))
map_municipios_2010_ext <- map_municipios_2010_ext %>%
  group_by(ine.prov.name) %>%
  mutate(Total = mean(Total, na.rm = TRUE)) %>%
  group_by(ine.prov.name, Total) %>%
  summarise(geometry = st_union(geometry), .groups = "drop")

map_municipios_2019_ext <- map_municipios %>%
  left_join(mun_data_2019_ext, by = c("name" = "Municipios"))
map_municipios_2019_ext <- map_municipios_2019_ext %>%
  group_by(ine.prov.name) %>%
  mutate(Total = mean(Total, na.rm = TRUE)) %>%
  group_by(ine.prov.name, Total) %>%
  summarise(geometry = st_union(geometry), .groups = "drop")

m5 <- map_municipios_2010_ext %>% pull(Total) %>% min(na.rm = TRUE)
m6 <- map_municipios_2019_ext %>% pull(Total) %>% min(na.rm = TRUE)
min_value_act <- min(m5, m6)
m7 <- map_municipios_2010_ext %>% pull(Total) %>% max(na.rm = TRUE)
m8 <- map_municipios_2019_ext %>% pull(Total) %>% max(na.rm = TRUE)
max_value_act <- max(m7, m8)


g3 <- ggplot(data = map_municipios_2010_ext) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray", limits = c(min_value_act, max_value_act), direction = -1) +  # Escala de color
  theme_minimal() +
  labs(title = "Porcentaje de Extranjeros, Año 2010",
       fill = "Porcentaje") +
  theme(plot.title = element_text(size = 10),
    legend.title = element_text(size = 8))


g4 <- ggplot(data = map_municipios_2019_ext) +
  geom_sf(aes(fill = Total), color = "white") +  # Colorear por el indicador
  scale_fill_viridis_c(option = "rocket", na.value = "lightgray",limits = c(min_value_act, max_value_act), direction = -1) +  # Escala de color
  theme_minimal() +
  labs(title = "Porcentaje de Extranjeros, Año 2019",
       fill = "Porcentaje") +
  theme(plot.title = element_text(size = 10),
    legend.title = element_text(size = 8))
...
* Tras el análisis de los datos, sí se puede ver la influencia de algunos eventos históricos. Se observa una alta tasa de desempleo (y baja de ocupación) en los años previos a 2014, posiblemente relacionado con la crisis de 2008. Los datos también reflejan los efectos del COVID-19, mostrando un repunte en la tasa de mortalidad en el año 2020.




* Además, se ha identificado un incremento en la proporción de población mayor de 65 años, acompañado de una disminución en la proporción de personas entre 15 y 64 años. Dichos comportamientos reflejan un proceso de envejecimiento demográfico. Asimismo, se ha constatado una tendencia decreciente en la tasa de natalidad a lo largo del tiempo. Por último, se han examinado diversos indicadores, como la edad media de la población, la tasa de actividad y la proporción de viviendas unipersonales, los cuales podrían emplearse para evaluar la situación demográfica, dado que presentan una alta correlación con factores como la proporción de personas en el rango de 15 a 64 años y la de mayores de 65 años.




* La tasa de desempleo no muestra una correlación significativa con ningún indicador demográfico en particular. Este hallazgo es especialmente interesante en el caso del porcentaje de extranjeros en la población, ya que podría esperarse que este factor tuviera algún impacto en la tasa de desempleo. Esto podría indicar que los extranjeros están bien integrados en el mercado laboral o que el desempleo puede estar más influenciado por otros factores, como pueden ser económicos, políticos o estructurales, que por características demográficas. 




* Al observar los mapas, se puede notar una clara relación entre los patrones geográficos y algunos indicadores. Por ejemplo, la tasa de desempleo muestra una distribución diferenciada entre el norte y el sur, con valores más altos en algunas regiones del sur de España. Además, el porcentaje de extranjeros también varía geográficamente, siendo más alto en el este del país. Estas diferencias reflejan variaciones notables entre las regiones, que se visualizan claramente en los mapas representados.
